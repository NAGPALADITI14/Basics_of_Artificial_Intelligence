...
int minimax(int depth, bool isMaximizing, int pass_score, int best_score)
{
    ...
    if (isMaximizing)
    {
        ...
        {
            play_board[move] = AI;
            int score = minimax(depth + 1, false, pass_score, best_score);
            play_board[move] = 2;

            bestMoveScore = max(score, bestMoveScore);
            pass_score = max(pass_score, bestMoveScore);
    ...
    {
        int bestMoveScore = numeric_limits<int>::max();
        for (int move : Generate_moves())
        {
            play_board[move] = human;
            int score = minimax(depth + 1, true, pass_score, best_score);
            play_board[move] = 2;

            bestMoveScore = min(score, bestMoveScore);
            best_score = min(best_score, bestMoveScore);
