...
struct Node
{
    ...
    Node(int id, int cost) : id(id), cost(cost), parent(nullptr) {}
};
...
void bestFirstSearch(const vector<vector<int>> &adjMatrix, int startId, int goalId)
{
    auto compare = [](Node *a, Node *b)
    { return a->cost > b->cost; };
    priority_queue<Node *, vector<Node *>, decltype(compare)> openList(compare);
    vector<bool> closedList(28, false);

    Node *startNode = new Node(startId, 0);
    ...
    while (!openList.empty())
    {
        ...
        {
            cout << "Goal node found: " << char('a' + currentNode->id) << "\n";
            cout << "Path: ";
            Path(currentNode);
            cout << "Goal\n";
            return;
        ...
        for (int i = 0; i < 28; ++i)
        {
            if (adjMatrix[currentNode->id][i] != 0 && !closedList[i])
            {
                Node *adjNode = new Node(i, adjMatrix[currentNode->id][i]);
...
void initializeMatrixFromText(vector<vector<int>> &adjMatrix, const string &text)
{
    for (size_t i = 0; i < text.length() - 1; ++i)
    {
        char currentChar = tolower(text[i]);
        char nextChar = tolower(text[i + 1]);
        if (isalpha(currentChar) && isalpha(nextChar))
        {
            int from = currentChar - 'a';
            int to = nextChar - 'a';
            adjMatrix[from][to] = 1; 
        }
    }
...
int main()
{
    ...
    initializeMatrixFromText(adjMatrix, text);
    int startId = 'e' - 'a'; 
    int goalId = 'g' - 'a';  
    bestFirstSearch(adjMatrix, startId, goalId);
    return 0;
}
